...
const actions = {
  SET_ARTICLE_LIST: (store, json) => {
    return request
      .post("/admin/api/article/list", json, store)
      .then(response => {
  ...
  SET_ARTICLE_DETAIL: (store, { id }) => {
    ...
    return request.get(`/admin/api/article/${id}`, store).then(response => {
  ...
  SET_SAVE_ARTICLE: (store, json) => {
    const { commit, dispatch, state } = store;
    return request
      .post("/admin/api/article/add", json, store)
      .then(response => {
  ...
  DELETE_ARTICLE: (store, { id }) => {
    // 鉴权 TODO
    const { commit, dispatch, state } = store;
    return request.get(`/admin/api/article/del/${id}`, store).then(response => {
